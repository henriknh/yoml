...
class TVShowsEventHandler(FileSystemEventHandler):
    """Logs all the events captured."""

    def on_moved(self, event):
        super(TVShowsEventHandler, self).on_moved(event)

        what = 'directory' if event.is_directory else 'file'
        logging.info("TVShows - Moved %s: from %s to %s", what, event.src_path,
                     event.dest_path)

    def on_created(self, event):
        super(TVShowsEventHandler, self).on_created(event)

        what = 'directory' if event.is_directory else 'file'
        logging.info("TVShows - Created %s: %s", what, event.src_path)

    def on_deleted(self, event):
        super(TVShowsEventHandler, self).on_deleted(event)

        what = 'directory' if event.is_directory else 'file'
        logging.info("TVShows - Deleted %s: %s", what, event.src_path)

    def on_modified(self, event):
        super(TVShowsEventHandler, self).on_modified(event)

        what = 'directory' if event.is_directory else 'file'
        logging.info("TVShows - Modified %s: %s", what, event.src_path)

class MoviesEventHandler(FileSystemEventHandler):
    """Logs all the events captured."""

    def on_moved(self, event):
        super(MoviesEventHandler, self).on_moved(event)

        what = 'directory' if event.is_directory else 'file'
        logging.info("Movies - Moved %s: from %s to %s", what, event.src_path,
                     event.dest_path)

    def on_created(self, event):
        super(MoviesEventHandler, self).on_created(event)

        what = 'directory' if event.is_directory else 'file'
        logging.info("Movies - Created %s: %s", what, event.src_path)

    def on_deleted(self, event):
        super(MoviesEventHandler, self).on_deleted(event)

        what = 'directory' if event.is_directory else 'file'
        logging.info("Movies - Deleted %s: %s", what, event.src_path)

    def on_modified(self, event):
        super(MoviesEventHandler, self).on_modified(event)

        what = 'directory' if event.is_directory else 'file'
        logging.info("Movies - Modified %s: %s", what, event.src_path)

class PicturesEventHandler(FileSystemEventHandler):
    """Logs all the events captured."""

    def on_moved(self, event):
        super(PicturesEventHandler, self).on_moved(event)

        what = 'directory' if event.is_directory else 'file'
        logging.info("Pictures - Moved %s: from %s to %s", what, event.src_path,
                     event.dest_path)

    def on_created(self, event):
        super(PicturesEventHandler, self).on_created(event)

        what = 'directory' if event.is_directory else 'file'
        logging.info("Pictures - Created %s: %s", what, event.src_path)

    def on_deleted(self, event):
        super(PicturesEventHandler, self).on_deleted(event)

        what = 'directory' if event.is_directory else 'file'
        logging.info("Pictures - Deleted %s: %s", what, event.src_path)

    def on_modified(self, event):
        super(PicturesEventHandler, self).on_modified(event)

        what = 'directory' if event.is_directory else 'file'
        logging.info("Pictures - Modified %s: %s", what, event.src_path)

class MusicEventHandler(FileSystemEventHandler):
    """Logs all the events captured."""

    def on_moved(self, event):
        super(MusicEventHandler, self).on_moved(event)

        what = 'directory' if event.is_directory else 'file'
        logging.info("Music - Moved %s: from %s to %s", what, event.src_path,
                     event.dest_path)

    def on_created(self, event):
        super(MusicEventHandler, self).on_created(event)

        what = 'directory' if event.is_directory else 'file'
        logging.info("Music - Created %s: %s", what, event.src_path)

    def on_deleted(self, event):
        super(MusicEventHandler, self).on_deleted(event)

        what = 'directory' if event.is_directory else 'file'
        logging.info("Music - Deleted %s: %s", what, event.src_path)

    def on_modified(self, event):
        super(MusicEventHandler, self).on_modified(event)

        what = 'directory' if event.is_directory else 'file'
        logging.info("Music - Modified %s: %s", what, event.src_path)
